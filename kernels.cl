 		    ...
 		    float omega)
{
  ...
  if (obstacles[ii * nx + jj])
  {
    cells[1*N + ii * nx + jj] = tmp_cells[3*N + ii * nx + jj];
    cells[2*N + ii * nx + jj] = tmp_cells[4*N + ii * nx + jj];
    cells[3*N + ii * nx + jj] = tmp_cells[1*N + ii * nx + jj];
    cells[4*N + ii * nx + jj] = tmp_cells[2*N + ii * nx + jj];
    cells[5*N + ii * nx + jj] = tmp_cells[7*N + ii * nx + jj];
    cells[6*N + ii * nx + jj] = tmp_cells[8*N + ii * nx + jj];
    cells[7*N + ii * nx + jj] = tmp_cells[5*N + ii * nx + jj];
    cells[8*N + ii * nx + jj] = tmp_cells[6*N + ii * nx + jj];
  }
  else
  {
    ...
    for (int kk = 0; kk < NSPEEDS; kk++)
      local_density += tmp_cells[kk*N + ii * nx + jj];

    /* x-component of velocity */
    float u_x = (tmp_cells[1*N + ii * nx + jj]
               + tmp_cells[5*N + ii * nx + jj]
               + tmp_cells[8*N + ii * nx + jj]
               - (tmp_cells[3*N + ii * nx + jj]
                  + tmp_cells[6*N + ii * nx + jj]
                  + tmp_cells[7*N + ii * nx + jj])) 
               / local_density;

    /* compute y velocity component */
    float u_y = (tmp_cells[2*N + ii * nx + jj]
               + tmp_cells[5*N + ii * nx + jj]
               + tmp_cells[6*N + ii * nx + jj]
               - (tmp_cells[4*N + ii * nx + jj]
                  + tmp_cells[7*N + ii * nx + jj]
                  + tmp_cells[8*N + ii * nx + jj])) 
               / local_density;
    ...
    for (int kk = 0; kk < NSPEEDS; kk++)
    {
      cells[kk*N + ii * nx + jj] = tmp_cells[kk*N + ii * nx + jj]
                                              + omega
                                              * (d_equ[kk] - tmp_cells[kk*N + ii * nx + jj]);
    }
  }
}
   ...
   int nx, int ny, float omega)
{
  ...
  if (obstacles[ii])
  {
    cells[1*N + ii] = tmp_cells[3*N + ii];
    cells[2*N + ii] = tmp_cells[4*N + ii];
    cells[3*N + ii] = tmp_cells[1*N + ii];
    cells[4*N + ii] = tmp_cells[2*N + ii];
    cells[5*N + ii] = tmp_cells[7*N + ii];
    cells[6*N + ii] = tmp_cells[8*N + ii];
    cells[7*N + ii] = tmp_cells[5*N + ii];
    cells[8*N + ii] = tmp_cells[6*N + ii];
  ...
  else
  {
    ...
    for (int kk = 0; kk < NSPEEDS; kk++)
    {
      cells[kk*N + ii] = tmp_cells[kk*N + ii]
                         + omega
                         * (d_equ[kk] - tmp_cells[kk*N + ii]);
    }
    ...
    /* velocity squared */
    local_sums[local_id] = sqrt(u_x * u_x + u_y * u_y) / local_density;
  }

//  float u = sqrt(u_sq);
//  float mask = !obstacles[ii] ? 1 : 0;
//  local_sums[local_id] = mask * u;
